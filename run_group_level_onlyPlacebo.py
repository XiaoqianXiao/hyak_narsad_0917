...
def run_group_level_wf(task, contrast, analysis_type, paths):
    ...
    if analysis_type == 'flameo':
        # always one‐sample FLAMEO but still supply a covsplit file 🔧
        wf = wf_flameo(
            output_dir = paths['result_dir'],
            use_covsplit = True,       # 🔧 now always True
            name = wf_name
        )
    else:
        wf = wf_randomise(
    ...
    if analysis_type == 'flameo':
        ...
        wf.inputs.inputnode.covsplit_file   = paths['grp_file']  # 🔧 ensure this exists
...
if __name__ == '__main__':
    ...
    paths = {
        ...
        'grp_file':     os.path.join(contrast_dir, 'design_files', 'design.grp'),  # 🔧 must be a valid VEST file
