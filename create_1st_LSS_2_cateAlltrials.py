...
Usage:
    ...
    python create_1st_LSS_2_cateAlltrials.py
    
    # Generate scripts for specific subjects only
    python create_1st_LSS_2_cateAlltrials.py --subjects N101 N102 N103
    
    # Generate scripts for specific tasks only
    python create_1st_LSS_2_cateAlltrials.py --tasks phase2 phase3
    
    # Generate scripts with custom SLURM settings
    python create_1st_LSS_2_cateAlltrials.py --account psych --partition cpu-g2-mem2x --memory 64G
    
    # Generate scripts for specific contrasts
    python create_1st_LSS_2_cateAlltrials.py --contrasts 1 2 3
    
    # Show help
    python create_1st_LSS_2_cateAlltrials.py --help
...
def process_subject_task(layout, subject, task, slurm_config, contrasts=None, dry_run=False):
    """
    ...
    try:
        ...
        if dry_run:
            print(f"  [DRY RUN] Would create SLURM script for {len(trial_ids)} trials")
            return 1
...
Examples:
    ...
    python create_1st_LSS_2_cateAlltrials.py --dry-run
        """
    )
    ...
    for subject in subjects_to_process:
        ...
        for task in tasks_to_process:
            ...
            scripts_created = process_subject_task(
                layout, subject, task, slurm_config, args.contrasts, args.dry_run
            )
