...
#SBATCH --mem=40G
#SBATCH --time=2:00:00
#SBATCH --output=/gscratch/scrubbed/fanglab/xiaoqian/NARSAD/work_flows/single_trial/{task}_sub_{sub}_%j.out
#SBATCH --error=/gscratch/scrubbed/fanglab/xiaoqian/NARSAD/work_flows/single_trial/{task}_sub_{sub}_%j.err


module load apptainer
apptainer exec -B /gscratch/fang:/data -B /gscratch/scrubbed/fanglab/xiaoqian:/scrubbed_dir {container_path} \\
    python3 /app/run_1st_level_single_trial.py --subject {sub} --task {task}
"""
...
def run_subject_workflow(sub, inputs, work_dir, output_dir):
    from first_level_workflows import first_level_single_trial_LSS_wf  # Assumes workflows.py is in /app
    workflow = first_level_single_trial_LSS_wf(inputs, output_dir)
    ...
    if args.subject:  # Run for a single subject
        ...
        for part in layout.get(invalid_filters='allow', **query):
            if part.entities['subject'] == args.subject and (not args.task or part.entities['task'] == args.task):
                ...
                task = entities['task']
                work_dir = os.path.join(scrubbed_dir, project_name, f'work_flows/single_trial/{task}')
